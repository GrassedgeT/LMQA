...
```
前端 (React) ←→ HTTP/RESTful API ←→ 后端 (Flask) ←→ 智能体服务 (Agent Service)
                ↓
            数据库 (PostgreSQL)
```
...
- `Redux Toolkit` 或 `Zustand`：状态管理（可选）
...
- **稳定可靠**：企业级数据库，数据安全性高
- **扩展性好**：支持向量扩展（pgvector），适合未来AI功能扩展
- **主流选择**：Python项目常用数据库
